...
document.write('<link rel="stylesheet" href="https://srwebrtc.herokuapp.com/getMediaElement.css">');
...
function getMediaElement(mediaElement, config) {
	...
	if (buttons.has('stop-others')) {
		var stopOthers = document.createElement('div');
		stopOthers.className = 'control stop-others';
		mediaControls.appendChild(stopOthers);

		stopOthers.onclick = function() {
			setTimeout(function() {
				if (mediaElementContainer.parentNode) {
					var mediaParent = mediaElementContainer.parentNode;
					var bitrateIndicatorEl = document.getElementById('bitrateIndicator');
					while (mediaParent.firstChild) {
						mediaParent.removeChild(mediaParent.firstChild);
					};
					mediaParent.appendChild(bitrateIndicatorEl);
					mediaParent.appendChild(mediaElementContainer);
				};
			}, 800);
		}
	}
